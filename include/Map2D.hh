...
class Map2D{
    public:
        ...
        int get(int x, int y){
            if(x<0||sizeX()<=x) return -1;
            if(y<0||sizeY()<=y) return -1;
            x+=subMapOffsetX;
            y+=subMapOffsetY;
            int subIndexX=x/SUBMAP_SIZE;
            int subIndexY=y/SUBMAP_SIZE;
            if(subMap[subIndexX][subIndexY]==nullptr) return -1;
        ...
        double getHeight(int x, int y){
            if(x<0||sizeX()<=x) return NAN;
            if(y<0||sizeY()<=y) return NAN;
            x+=subMapOffsetX;
            y+=subMapOffsetY;
            int subIndexX=x/SUBMAP_SIZE;
            int subIndexY=y/SUBMAP_SIZE;
            if(subMapHeight[subIndexX][subIndexY]==nullptr) return NAN;
            int indexX=x%SUBMAP_SIZE;
            int indexY=y%SUBMAP_SIZE;
        ...
        double getHeightTop(int x, int y){
            ...
            int subIndexX=x/SUBMAP_SIZE;
            int subIndexY=y/SUBMAP_SIZE;
            if(subMapHeightTop[subIndexX][subIndexY]==nullptr) return NAN;
            int indexX=x%SUBMAP_SIZE;
            int indexY=y%SUBMAP_SIZE;
        ...
        double getSlope(int x, int y){
            ...
            int subIndexX=x/SUBMAP_SIZE;
            int subIndexY=y/SUBMAP_SIZE;
            if(subMapSlope[subIndexX][subIndexY]==nullptr) return NAN;
            int indexX=x%SUBMAP_SIZE;
            int indexY=y%SUBMAP_SIZE;
        ...
        void set(int x, int y, int value){
            ...
            if(subMap[subIndexX][subIndexY]==nullptr)subMap[subIndexX][subIndexY]=generateSubMapInt();
        ...
        void setHeight(int x, int y, double value){
            int subIndexX, subIndexY, indexX, indexY;
            getIndexSubMap(x,y,subIndexX,subIndexY,indexX,indexY);
            if(subMapHeight[subIndexX][subIndexY]==nullptr)subMapHeight[subIndexX][subIndexY]=generateSubMapDouble();
        ...
        void setHeightTop(int x, int y, double value){
            int subIndexX, subIndexY, indexX, indexY;
            getIndexSubMap(x,y,subIndexX,subIndexY,indexX,indexY);
            if(subMapHeightTop[subIndexX][subIndexY]==nullptr)subMapHeightTop[subIndexX][subIndexY]=generateSubMapDouble();
        ...
        void setSlope(int x, int y, double value){
            int subIndexX, subIndexY, indexX, indexY;
            getIndexSubMap(x,y,subIndexX,subIndexY,indexX,indexY);
            if(subMapSlope[subIndexX][subIndexY]==nullptr)subMapSlope[subIndexX][subIndexY]=generateSubMapDouble();
    ...
    private:
        ...
        //Get the index for map and submap, if index x y is outside of map, map is expanded 
        void getIndexSubMap(int x, int y, int& subIndexX, int& subIndexY, int& indexX, int& indexY){
        ...
        void growSubMaps(int growX, int growY){
            ...
            if(subMap.size()==0){
                ...
                if(growX!=0)growX=growX/abs(growX)*max(int(subMap.size()),abs(growX));
                if(growY!=0)growY=growY/abs(growY)*max(int(subMap[0].size()),abs(growY));
