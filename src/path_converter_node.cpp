...
class PathConverter
{
	...
	PathConverter(){
		...
		subHeight = nh.subscribe("heightMap", 1, &PathConverter::heightCallback, this);
	...
	void updateCollisionArea(double newResulution){
		if(newResulution==currentResulution) return;
		currentResulution=newResulution;
		if(currentResulution<=0.0) return;
	...
	//Generate collision shape as half sphere that will be mirrored around xy plane
	void generateCollisionSphere(){
		...
		for(int x = -radiusSize; x<=radiusSize; x++){
			for(int y = -radiusSize; y<=radiusSize; y++){
				if(sqrt(x*x+y*y)>radiusSize) continue;
	...
	void updatePath(){
		...
		if(collsionShape) solveCollisionUAV(&outPath);
	...
	void setPathHeight3D(nav_msgs::Path* path){
		...
		for(int i=0; i<path->poses.size();i++){
			if(i+pathSmothingLength<path->poses.size()){
				...
				if(isnan(height)) continue;
				heightList.insert(heightList.begin(),height);
				if(heightList.size()>pathSmothingLength*2) heightList.pop_back();
			}
			double pathHeight=-INFINITY;
			for(double h : heightList) pathHeight=max(pathHeight,h);
			if(isinf(pathHeight)) continue;
	...
	double getHeight(int x, int y){
		...
		if(index<0||index>=hMap.bottom.size()) return NAN;
	...
	double getHeightTop(int x, int y){
		int index=x+y*hMap.info.width;
		if(index<0||index>=hMap.top.size()) return NAN;
