...
MapConverter::MapConverter(double resolution, int slopeEstimationSize, double minimumZ, int minimumOccupancy){
...
void MapConverter::updateMap(vector<voxel> vMap, vector<double> minMax){
    ...
    for(voxel v : vMap){
        ...
        for(int x=0; x<sizeIndex;x++){
            for(int y=0; y<sizeIndex;y++){
                ...
                heightRange hr={v.position.z+v.halfSize+resolution*0.001,v.position.z-v.halfSize-resolution*0.001};
                hMap.addRange(posX,posY,hr,v.occupied,v.position.x-v.halfSize+resolution*x,v.position.y-v.halfSize+resolution*y);
            }
        }
    }
    ...
    for(int x=1;x<xSize-1;x++){
        for(int y=1;y<ySize-1;y++){           
            ...
            map.setHeight(hMap.posMap[x][y].x,hMap.posMap[x][y].y,hMap.free[x][y].back().bottom);
            map.setHeightTop(hMap.posMap[x][y].x,hMap.posMap[x][y].y,hMap.free[x][y][0].top);
        }
    }
    ...
    for(int x=2;x<xSize-2;x++){
        for(int y=2;y<ySize-2;y++){           
            ...
            for(auto d : DIRECTIONS){
                if(hMap.free[x+d.x][y+d.y].size()!=0) continue;
